...
def GetData(data_folder, otherinfo):
    ...
    if float(wavelengths[0])>900:
        ...
        sensors = "FX10"
        min_area = 100000
    return whiteimg_data, darkimage_data, rawimg_data, wavelengths, sensors,min_area
...
def labelimage_fixed(mask, row=4, col=2,min_area = 60000): #调整了最小圆面积的阈值，排除其他背景图像干扰
    mask=mask.astype(np.uint8)
...
def main(totalfolder,outpath, pic_scale, output_csv, output_csvC, incorrect_file, outMaskPath, outRawPath, overnum, overrepeat):
    ...
    for filename in file_list:
        # filename = "sgz_259-266-1.1_2025-06-19_06-34-46"
        ...
        whiteimg_data, darkimage_data, rawimg_data, wavelengths, sensors, min_area = GetData(data_folder, filename)
        ...
        label_img, sorted_info,used = labelimage_fixed(mask,min_area)
        if label_img.max() > 8:
...
if __name__ == "__main__":
    ...
    totalfolder = Path(r'Z:/00.rawdata/FX17_SGZ')
    outname = "FX10_SGZ_result"
    outpath = "Z:/test/"
    ...
    if Path(output_csvC).exists():
        df =  pd.read_csv(output_csvC, sep=",")
